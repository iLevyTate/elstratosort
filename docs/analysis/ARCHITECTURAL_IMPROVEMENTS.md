...
class FileOrganizationSaga {
    ...
    async execute(operations) {
        ...
        logger.info(`[Saga] Started transaction ${txId} with ${operations.length} operations`);
        ...
        try {
            for (const op of operations) {
                ...
                logger.debug(`[Saga] Completed step ${stepNumber}/${operations.length}`);
            }
            ...
            logger.error(`[Saga] Transaction ${txId} failed at step ${stepNumber}`, error);
    ...
    async _rollback(txId) {
        ...
        for (const op of operations) {
            try {
                ...
                logger.error(`[Saga] Rollback failed for step ${op.step_number}`, rollbackError);
...
async function handleBatchOrganize({ operation, logger, getServiceIntegration, getMainWindow }) {
    if (!saga) {
        initializeSaga();
    }
    ...
    if (result.success) {
        ...
        try {
            ...
            logger.warn('[FileOps] Database update failed (non-fatal)', { error: dbError.message });
        }
...
class ServiceContainer extends EventEmitter {
    ...
    async _initialize(name) {
        ...
        try {
            ...
            logger.error(`[ServiceContainer] Service initialization failed: ${name}`, error);
...
import { setSelectedFiles, removeSelectedFile } from '../store/slices/filesSlice';
...
class StratoSortError extends Error {
    constructor(message, code, context = {}, userMessage = null, recoveryActions = []) {
        super(message);
