...
// Context is now set per-log call using the logger's prefix feature
  ...
  const loadSmartFolders = async () => {
    try {
      ...
      if (!Array.isArray(folders)) {
        logger.warn('Received non-array response', { response, folders });
        setSmartFolders([]);
        return;
      }
          ...
          >
            ...
            onClick={async () => {
              ...
              if (currentFolders.length === 0) {
                ...
                dispatch(advancePhase({ targetPhase: PHASES.DISCOVER }));
              }
            }}
            variant="primary"
            className="w-full sm:w-auto"
