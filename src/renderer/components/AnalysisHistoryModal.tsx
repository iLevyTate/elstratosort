...
function AnalysisHistoryModal({ onClose, analysisStats, setAnalysisStats }) {
  ...
  const loadAnalysisData = async () => {
    ...
    try {
      ...
      // Handle both wrapped { success, data } and legacy format
      const stats = statsResponse?.data ?? statsResponse ?? {};
      const history = historyResponse?.data ?? (Array.isArray(historyResponse) ? historyResponse : []);
  ...
  const searchHistory = async () => {
    ...
    try {      const response = await window.electronAPI.analysisHistory.search(
        searchQuery,
        { limit: 200 },
      );
