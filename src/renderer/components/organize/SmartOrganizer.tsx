...
interface SmartOrganizerProps {
  ...
  onOrganize?: (suggestions: Record<string, unknown>) => void;
  onCancel?: () => void;
}
...
function SmartOrganizer({
  ...
  const [batchSuggestions, setBatchSuggestions] = useState<BatchSuggestionResult | null>(null);
  const [folderImprovements, setFolderImprovements] = useState<Array<{ description?: string }>>([]);
  const [acceptedSuggestions, setAcceptedSuggestions] = useState<Record<string, unknown>>({});
  ...
  const handleQuickOrganize = () => {
    ...
    const highConfidenceSuggestions: Record<string, unknown> = {};
  ...
  const handleAcceptSuggestion = (file: FileData, suggestion: unknown) => {
  ...
  const handleRejectSuggestion = (file: FileData, suggestion: unknown) => {
    // Record feedback for learning
    (async () => {
      try {
        await window.electronAPI.suggestions.recordFeedback(
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Stats Bar */}
      {(Object.keys(suggestions).length > 0 || batchSuggestions) && (
  ...
  function calculateAverageConfidence() {
    if (files.length === 1) {
      const suggestion = suggestions[files[0].path];
      return suggestion?.confidence ? Math.round(suggestion.confidence * 100) : 0;
    } else if (batchSuggestions) {
      const avg =
