...
interface SuggestionsWithPrimary {
  primary?: Suggestion;
...
interface OrganizationPreviewProps {
  files?: FileData[];
  strategy?: Strategy;
  suggestions?: Suggestion[] | SuggestionsWithPrimary | Record<string, unknown>;
  onConfirm?: () => void;
  onCancel?: () => void;
}
...
function OrganizationPreview({
  ...
  const [previewTree, setPreviewTree] = useState<Record<string, PreviewFolder>>({});
  ...
  useEffect(() => {
    ...
    const tree: Record<string, PreviewFolder> = {};
    const folderCount = new Set<string>();
    let movedCount = 0;
    let renamedCount = 0;
    ...
    files.forEach((file, index) => {
      ...
      } else if (suggestions && typeof suggestions === 'object' && 'primary' in suggestions) {
        suggestion = (suggestions as SuggestionsWithPrimary).primary;
      } else {
      ...
      if (!tree[folderPath]) {
        tree[folderPath] = {
          name: suggestion.folder || folderPath,
          path: folderPath,
          files: [],
          confidence: 0,
        };
