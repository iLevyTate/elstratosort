...
class ModelManager {
  ...
  private modelCapabilities: Map<string, Partial<ModelCapabilities>>;
  private lastHealthCheck: string | null;
  private configPath: string;
  private initialized: boolean;
  private _initPromise: Promise<boolean> | null;
  private _isInitializing: boolean;
  private modelCategories: Record<string, string[]>;
  private fallbackPreferences: string[];
  ...
  async discoverModels(): Promise<OllamaModel[]> {
    try {
      const response = (await this.ollamaClient.list()) as OllamaListResponse;
   ...
   */
  analyzeModelCapabilities(model: OllamaModel): Partial<ModelCapabilities> {
    ...
    for (const [capability, patterns] of Object.entries(this.modelCategories)) {
      // Only set boolean capabilities, not size/modified
      if (capability === 'text' || capability === 'vision' || capability === 'code' || capability === 'chat') {
        capabilities[capability] = patterns.some(
          (pattern) => modelName.includes(pattern.toLowerCase()),
        );
      }
    }
   ...
   */
  async ensureWorkingModel(): Promise<string> {
   ...
   */
  async testModel(modelName: string, timeout: number = 10000): Promise<boolean> {
    ...
    try {
      ...
      // Note: Ollama client doesn't support AbortSignal directly in GenerateRequest
      // Cancellation is handled via timeout promise race
      ...
      logger.debug(
        `[ModelManager] Model ${modelName} failed test: ${(error as Error).message}`,
      );
      return false;
    } finally {
   ...
   */
  getBestModelForTask(task: string = 'text'): string | null {
    if (!this.selectedModel) {
      return null;
    }
   ...
   */
  async setSelectedModel(modelName: string): Promise<void> {
   ...
   */
  getModelInfo(modelName: string | null = null): ModelInfo | null {
  ...
  async generateWithFallback(
    ...
    for (const modelName of modelsToTry) {
      try {
        ...
        logger.debug(
          `[ModelManager] Model ${modelName} failed: ${(error as Error).message}`,
        );
        continue;
      }
    }
  ...
  async loadConfig(): Promise<void> {
    try {
      ...
      const config: ModelConfig = JSON.parse(data);
   ...
   */
  getAllModelsWithCapabilities(): ModelInfo[] {
