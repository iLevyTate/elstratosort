...
interface UndoRedoServiceOptions {
  maxActions?: number;
  maxMemoryMB?: number;
  maxBatchSize?: number;
}
...
class UndoRedoService {
  ...
  private currentMemoryEstimate: number;

  constructor(options: UndoRedoServiceOptions = {}) {
  ...
  async safeMove(sourcePath: string, destinationPath: string): Promise<void> {
   ...
   * @private
   */
  private _estimateActionSize(action: UndoAction): number {
   ...
   * @private
   */
  private _recalculateMemoryEstimate(): void {
  ...
  async recordAction(actionType: string, actionData: ActionData): Promise<string> {
  ...
  async executeReverseAction(action: UndoAction): Promise<void> {
    switch (action.type) {
      case 'FILE_MOVE':
  ...
  async executeForwardAction(action: UndoAction): Promise<void> {
    switch (action.type) {
      case 'FILE_MOVE':
  ...
  async reverseFileOperation(operation: FileOperation): Promise<void> {
  ...
  async executeFileOperation(operation: FileOperation): Promise<void> {
    switch (operation.type) {
      case 'move':
  ...
  async createBackup(filePath: string): Promise<string> {
    ...
    try {
      ...
      if (sourceStats.size !== backupStats.size) {
        await fs.unlink(backupPath).catch((unlinkError: any) => {
  ...
  getActionDescription(actionType: string, actionData: ActionData): string {
    switch (actionType) {
      case 'FILE_MOVE':
  ...
  getActionHistory(limit = 10): ActionHistoryItem[] {
  ...
  getRedoHistory(limit = 10): ActionHistoryItem[] {
    const history = this.actions.slice(
      this.currentIndex + 1,
