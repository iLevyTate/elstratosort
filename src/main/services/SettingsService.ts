...
import { backupAndReplace } from '../../shared/atomicFileOperations';
import {
  validateSettings,
  sanitizeSettings,
} from '../../shared/settingsValidation';
...
class SettingsService {
  ...
  async save(settings) {
    ...
    return this._withMutex(async () => {
      ...
      if (!validation.valid) {
        const error = new Error('Invalid settings provided') as ValidationError;
      ...
      try {
        ...
        if (!validation.valid) {
          const error = new Error(
            'Backup contains invalid settings',
          ) as ValidationError;
          error.validationErrors = validation.errors;
          error.validationWarnings = validation.warnings;
          throw error;
        ...
        return {
          ...
          validationWarnings: validationError.validationWarnings,
        };
      }
    });
  }
