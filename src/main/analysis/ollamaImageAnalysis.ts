...
Required fields:
- content_type: people|landscape|document_scan|screenshot|diagram|product|other
- has_text: true/false
- text_content: visible text if any (max 200 chars)
- category: one of folders above
- keywords: [5-7 descriptive terms]
- purpose: what image shows (5-10 words)
- project: main subject (2-5 words)
- date: YYYY-MM-DD from visible text (or null)
- confidence: 60-100
- suggestedName: descriptive_filename`;
    ...
    // CRITICAL FIX: Add timeout wrapper to prevent indefinite hangs (like documentLlm.js)
    const IMAGE_ANALYSIS_TIMEOUT = 120000; // 2 minutes for vision models (slower than text)
    let timeoutId;
    let response;
    try {
      ...
      response = await Promise.race([
        ...
        new Promise((_, reject) => {
          ...
          try {
            ...
            // Silently ignore if unref not available
          }
        }),
      ]);
    } finally {
