...
class AppLifecycle {
  ...
  async onReady() {
    try {
      // PERFORMANCE FIX: Register services and basic IPC early
      logger.info('[STARTUP] Registering services in container');
      registerServices();

      // Load settings early (fast file read)
      ...
      // Load custom folders early (fast file read)
      try {
        this.customFolders = await loadCustomFolders();
        logger.info(
          `[STARTUP] Loaded ${this.customFolders.length} custom folders`,
        );
      } catch (error) {
      ...
      // Register IPC handlers EARLY so renderer can start making calls
      this.registerIpc();
      ...
      // Create Window early (show UI while services initialize)
      this.windowManager.createOrRestore();
      this.handleSettingsChanged(this.currentSettings);
      ...
      // BACKGROUND: Initialize services after window is visible
      // This makes the app appear faster while services start in background
  ...
  /**
   * Initialize services in background after window is visible
   * This improves perceived startup time
   */
  async initializeServicesBackground() {
    try {
      // Initialize critical services (this can take a few seconds)
      logger.info('[STARTUP-BG] Initializing critical services...');
      await initializeCriticalServices();
      logger.info('[STARTUP-BG] Critical services initialized');

      // Initialize ServiceIntegration
      this.serviceIntegration = new ServiceIntegration();
      await this.serviceIntegration.initialize();
      logger.info('[STARTUP-BG] ServiceIntegration initialized');

      // Run Startup Manager (health checks, etc.)
      const startupManager = getStartupManager();
      let startupResult;
      try {
        const { TIMEOUTS } = await import(
          '../../shared/performanceConstants.js'
        );
        const startupPromise = startupManager.startup();
        const timeoutPromise = new Promise((_, reject) =>
          setTimeout(
            () => reject(new Error('Startup manager timeout')),
            TIMEOUTS.SERVICE_STARTUP || 30000,
          ),
        );
        startupResult = await Promise.race([startupPromise, timeoutPromise]);
        logger.info('[STARTUP-BG] Startup manager completed');
      } catch (error: any) {
        logger.error('[STARTUP-BG] Startup manager failed:', error);
      ...
      logger.info('[STARTUP-BG] Background initialization complete');
    } catch (error) {
      logger.error('[STARTUP-BG] Background initialization failed:', error);
    }
  }
  ...
  handleCommandLineArgs() {
    ...
    if (args.includes('--open-documents')) {
      try {
        ...
        logger.error('Failed to open documents', e);
      }
    }
  ...
  setupJumpList() {
    if (process.platform === 'win32') {
      try {
        ...
        logger.error('Failed to set Jump List', e);
      }
    }
  }
  ...
  async installDevTools() {
    if (process.env.REACT_DEVTOOLS === 'true') {
      try {
        ...
        logger.warn('Failed to install React DevTools', e);
      }
    }
  }
  ...
  async checkIncompleteBatches() {
    ...
    setTimeout(() => {
      try {
        resumeIncompleteBatches(this.serviceIntegration, logger, () =>
          this.windowManager.getMainWindow(),
        );
      } catch (e) {
  ...
  async verifyModels() {
    const { default: ModelVerifier } = await import(
      '../services/ModelVerifier.js'
    );
    ...
    (async () => {
      try {
        ...
        if (cfg.selectedVisionModel)
          setOllamaVisionModel(cfg.selectedVisionModel);
        if (cfg.selectedEmbeddingModel)
          setOllamaEmbeddingModel(cfg.selectedEmbeddingModel);
      } catch (error) {
  ...
  setupAutoUpdater() {
    ...
    autoUpdater
      ...
      .catch((e) => logger.error('Update check failed', e));
  }
