...
const electronAPI = {
  files: {
    select: jest
      .fn()
      .mockResolvedValue({ canceled: false, filePaths: ['/test/file.txt'] }),
    selectDirectory: jest
      .fn()
      .mockResolvedValue({ canceled: false, filePaths: ['/test/dir'] }),
  ...
  analysis: {
    document: jest
      .fn()
      .mockResolvedValue({ suggestedName: 'document.txt', category: 'docs' }),
    image: jest
      .fn()
      .mockResolvedValue({ suggestedName: 'image.png', category: 'images' }),
    extractText: jest
      .fn()
      .mockResolvedValue({ success: true, text: 'extracted text' }),
  },
  analysisHistory: {
    get: jest.fn().mockResolvedValue([]),
